#!/usr/bin/env python3
"""Client side executable for sshedit.

This program needs to be running on the client machine and the SSH client needs
to be properly set up to allow sshedit to function properly.
"""
...
class SocketRequestHandler(socketserver.BaseRequestHandler):
	...
	def get_line(self):
		...
		try:
			...
			line_length = -1
		while line_length == -1:
			self.data += self.request.recv(sshedit.BUFFER_SIZE)
			line_length = self.data.find(b'\n')
	...
	def handle(self):
		...
		if protocol_version != b'1':
			sshedit.print_err('Unknown protocol version.')
			return
		...
		try:
			...
			sshedit.print_err('Length cannot be', length)
			return
...
def main(args):
	...
	sshedit_dir = tempfile.TemporaryDirectory(prefix='sshedit-')
	# TODO: Allow manually setting the socket location.
	socket_address = args.socketaddress or sshedit_dir.name
	print('export SSHEDIT_SOCK=%s' % socket_address)
	if socket_address == sshedit_dir.name:
